...
- [Подходы к решению задачи](#подходы-к-решению-задачи)
    ...
    - [YOLO + padding](#yolo--padding)
        ...
        - [Проблемы данного решения](#проблемы-данного-решения-1)
    - [YOLO + padding + classification](#yolo--padding--classification)
        - [Размышления](#размышления-2)
        - [Гипотеза](#гипотеза-1)
        - [Логика](#логика-3)
        - [Плюсы YOLO + padding + classification](#плюсы-yolo--padding--classification)
        - [Минусы YOLO + padding + classification](#минусы-yolo--padding--classification)
        - [Проблемы данного решения](#проблемы-данного-решения-2)
    - [RF-DETR + classification](#rf-detr--classification)
        - [Размышления](#размышления-3)
        - [Гипотеза](#гипотеза-2)
        - [Логика](#логика-4)
        - [P.S.](#ps)
...
Код реализации лежит в ```nbs/yolo_padding_classif.ipynb```.

#### Размышления
В ноутбуке ```nbs/yolo_padding.ipynb``` был взят YOLO и перед детекцией был добавлен паддинг, затем прогнал через YOLO изображения и посчитал задетектированные книги, но данный подход имел свои слабые места описанные **в самом конце ноутбука**. 
